...
_BASIC_BUNDLE_SCRIPTS = [
    "HANA_Backups_BackupRuns_2.00.070+.txt",
    "HANA_Backups_SizeEstimation_1.00.120+.txt",
    "HANA_Configuration_Overview_2.00.040+.txt",
    "HANA_Configuration_Overview_2.00.080+.txt",
    "HANA_Configuration_Parameters_2.00.040+.txt",
    "HANA_Configuration_PatchLevel.txt",
    "HANA_Configuration_Timezones_2.00.040+.txt",
    "HANA_Connections_Statistics_2.00.060+.txt",
    "HANA_Consistency_CheckTableConsistency_Executions_2.00.040+.txt",
    "HANA_Consistency_CheckTableConsistency_Results_2.00.040+.txt",
    "HANA_Consistency_CheckTableConsistency_Tables_2.00.010+.txt",
    "HANA_Disks_Data_Overview_2.00.030+.txt",
    "HANA_Disks_Data_Partitions_2.00.040+.txt",
    "HANA_Disks_DiskUsage_2.00.040+.txt",
    "HANA_Disks_Overview.txt",
    "HANA_Hosts_FileSystems_2.00.020+.txt",
    "HANA_Hosts_Overview.txt",
    "HANA_IO_DiskDetails_2.00.020+.txt",
    "HANA_IO_KeyFigures_Details_2.00.060+.txt",
    "HANA_IO_Savepoints_2.00.060+.txt",
    "HANA_Locks_Transactional_LockWaits_2.00.060+.txt",
    "HANA_Locks_Transactional_LockWaits_PerObject.txt",
    "HANA_Memory_Components_2.00.060+.txt",
    "HANA_Memory_Heap_Allocations_2.00.060+.txt",
    "HANA_Memory_SQLCache_TopConsumers_2.00.053+.txt",
    "HANA_Memory_TopConsumers_2.00.060+.txt",
    "HANA_Replication_SystemReplication_Status.txt",
    "HANA_Resources_CPUAndMemory_2.00.060+.txt",
    "HANA_SQL_ExpensiveStatements_2.00.040+.txt",
    "HANA_StatisticsServer_Alerts_FrequentAlerts.txt",
    "HANA_StatisticsServer_Alerts_History.txt",
    "HANA_Tables_ColumnStore_Columns_2.00.060+.txt",
    "HANA_Tables_ColumnStore_Partitions_2.00.070+.txt",
    "HANA_Tables_LargestTables_2.00.060+.txt",
    "HANA_Tables_TopGrowingTables_Records_History_2.00.060+.txt",
    "HANA_Tables_TopGrowingTables_Size_History_2.00.060+.txt",
]

_bundle_state = {
    "running": False, "progress": 0, "total": 0,
    "current": "", "done": False, "filename": "",
    "ok_count": 0, "skip_count": 0, "skipped": []
}
_bundle_lock = threading.Lock()


def _run_basic_bundle(conn):
    from datetime import datetime
    date_str = datetime.now().strftime("%Y%m%d")
    out_name  = f"Consolidated_minichecks_basic_bundle_{date_str}.txt"
    out_path  = os.path.join(_OUTPUT_DIR, out_name)
    total     = len(_BASIC_BUNDLE_SCRIPTS)

    with _bundle_lock:
        _bundle_state.update({
    ...
    for i, fname in enumerate(_BASIC_BUNDLE_SCRIPTS):
        with _bundle_lock:
            _bundle_state["progress"] = i
            _bundle_state["current"]  = fname
        ...
        try:
            ...
            skip_count += 1
            skipped.append(f"{fname} (read error: {str(exc)[:80]})")
            continue
        ...
        if not sql_clean:
            skip_count += 1
            skipped.append(f"{fname} (empty SQL)")
            continue
        ...
        try:
            ...
            if cursor.description:
                ...
                cursor.close()
                csv_lines = [",".join(_q(c) for c in columns)]
                for row in rows:
                    csv_lines.append(",".join(_q("" if v is None else v) for v in row))
                sections.append((fname, "\n".join(csv_lines)))
            else:
                cursor.close()
                sections.append((fname, "(No result set returned)"))
            ok_count += 1
        except Exception as exc:
            skip_count += 1
            skipped.append(f"{fname} (SQL error: {str(exc)[:150]})")

    with _bundle_lock:
        _bundle_state["progress"] = total
        _bundle_state["current"]  = ""

    try:
        os.makedirs(_OUTPUT_DIR, exist_ok=True)
        with open(out_path, "w", encoding="utf-8") as out_f:
            out_f.write("SAP HANA SQL Console - Basic Bundle Mini Checks\n")
            out_f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            out_f.write(f"Scripts executed successfully: {ok_count}   Skipped: {skip_count}\n")
            if skipped:
                out_f.write("Skipped:\n")
                for s in skipped:
                    out_f.write(f"  - {s}\n")
            out_f.write("=" * 80 + "\n\n")
            for sec_fname, content in sections:
                out_f.write("=" * 80 + "\n")
                out_f.write(f"FILE: {sec_fname}\n")
                out_f.write("=" * 80 + "\n")
                out_f.write(content)
                out_f.write("\n\n")
        with _bundle_lock:
            _bundle_state.update({
                "running": False, "done": True, "filename": out_name,
                "ok_count": ok_count, "skip_count": skip_count, "skipped": skipped
            })
    except Exception as exc:
        with _bundle_lock:
            _bundle_state.update({
                "running": False, "done": True, "filename": "",
                "ok_count": ok_count, "skip_count": skip_count,
                "skipped": skipped, "error": str(exc)
            })
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == "/api/basic_bundle_status":
            with _bundle_lock:
                state = dict(_bundle_state)
    ...
    def do_POST(self):
        ...
        elif path == "/api/basic_bundle":
            with _conn_lock:
                conn = _conn
            if conn is None:
                return self._send_json({"ok": False, "error": "Not connected."})
            with _bundle_lock:
                if _bundle_state["running"]:
                    return self._send_json({"ok": False, "error": "A bundle execution is already running."})
            threading.Thread(target=_run_basic_bundle, args=(conn,), daemon=True).start()
...
/* ── Row limit radio group ── */
#row-limit-group { display: flex; align-items: center; gap: 6px; }
#row-limit-group .rl-label { font-size: 10px; color: var(--muted); text-transform: uppercase; letter-spacing: .5px; }
.rl-opt { display: flex; align-items: center; gap: 2px; cursor: pointer; }
.rl-opt input[type=radio] { accent-color: var(--accent); cursor: pointer; margin: 0; }
.rl-opt span { font-size: 11px; color: var(--text); white-space: nowrap; }

/* ── Bundle modal ── */
#bundle-overlay { position: fixed; inset: 0; background: rgba(15,30,55,.6);
  display: none; align-items: center; justify-content: center;
  z-index: 300; backdrop-filter: blur(4px); }
#bundle-overlay.show { display: flex; }
#bundle-box { background: var(--surf); border: 1px solid var(--border);
  border-radius: 10px; padding: 28px 32px; width: 540px;
  box-shadow: 0 16px 48px rgba(0,0,0,.18); display: flex; flex-direction: column; gap: 12px; }
#bundle-box h2 { font-size: 15px; color: var(--accent); text-align: center; }
#bundle-progress-wrap { background: var(--surf2); border-radius: 6px; height: 10px; overflow: hidden; }
#bundle-progress-bar { height: 100%; background: var(--accent); width: 0%;
  transition: width .4s ease; border-radius: 6px; }
#bundle-counter { font-size: 12px; color: var(--muted); text-align: center; }
#bundle-current { font-size: 12px; color: var(--text); text-align: center;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-height: 18px; }
#bundle-status { font-size: 12px; color: var(--muted); text-align: center; min-height: 18px; }
#bundle-skipped { font-size: 11px; color: var(--warn); max-height: 80px;
  overflow-y: auto; background: var(--surf2); border-radius: 4px;
  padding: 6px 8px; display: none; scrollbar-width: thin; }
#bundle-actions { display: flex; justify-content: center; }
</style>
</head>
<body>
...
<!-- Header -->
<div id="header">
  <div class="logo">SAP HANA <span>SQL Console</span> <small>v6.0</small></div>
  ...
  <button id="btn-clear-csvs" onclick="clearOutputCSVs()"
    style="padding:4px 10px;border-radius:5px;border:1px solid rgba(220,53,69,.4);background:rgba(220,53,69,.08);color:#ffaaaa;cursor:pointer;font-size:12px;display:none;">
    &#128465; Clear Outputs
  </button>
  <button id="btn-bundle" onclick="startBasicBundle()"
    style="padding:4px 10px;border-radius:5px;border:1px solid rgba(34,197,94,.4);background:rgba(34,197,94,.08);color:#86efac;cursor:pointer;font-size:12px;display:none;">
    &#9654;&#9654; Basic Bundle
  </button>
  <div id="conn-badge">
...
<div id="app">
  ...
  <div id="left-pane">
    <div class="toolbar">
      ...
      <div class="tb-sep"></div>
      <div id="row-limit-group">
        <span class="rl-label">Rows:</span>
        <label class="rl-opt"><input type="radio" name="rowLimit" value="500" checked/><span>500</span></label>
        <label class="rl-opt"><input type="radio" name="rowLimit" value="1000"/><span>1000</span></label>
        <label class="rl-opt"><input type="radio" name="rowLimit" value="0"/><span>All</span></label>
      </div>
    </div>
    <div id="editor-wrap">
...
<div id="about-overlay" onclick="if(event.target===this)hideAbout()">
  <div id="about-box">
    ...
    <div class="about-version">Version 6.0</div>
    <hr/>
    <div class="about-row"><span class="lbl">Version</span><span class="val">6.0</span></div>
    <div class="about-row"><span class="lbl">Date</span><span class="val">2026-08-12</span></div>
...
<!-- Bundle progress modal -->
<div id="bundle-overlay">
  <div id="bundle-box">
    <h2>&#9654;&#9654; Gather Basic Bundle</h2>
    <div id="bundle-progress-wrap"><div id="bundle-progress-bar"></div></div>
    <div id="bundle-counter">0 / 0</div>
    <div id="bundle-current">Initializing&hellip;</div>
    <div id="bundle-status"></div>
    <div id="bundle-skipped"></div>
    <div id="bundle-actions">
      <button class="btn btn-ghost" id="btn-bundle-close" onclick="hideBundle()" disabled>Close</button>
    </div>
  </div>
</div>
...
function onConnected(info) {
  ...
  $('btn-clear-csvs').style.display = 'inline-flex';
  $('btn-bundle').style.display = 'inline-flex';
  loadScriptList();
  $('sql-editor').focus();
}
...
async function doDisconnect() {
  ...
  $('btn-clear-csvs').style.display = 'none';
  $('btn-bundle').style.display = 'none';
  show('modal-overlay');
}
...
async function runQuery() {
  ...
  try {
    var r = await post('/api/execute', {sql: sql, row_limit: getRowLimit()});
    if (r.ok) {
      if (r.type === 'rows') {
        renderTable(r.columns, r.rows);
        var cap = r.capped ? '  ⚠ capped at ' + (r.row_limit || 500) : '';
...
function getRowLimit() {
  var sel = document.querySelector('input[name="rowLimit"]:checked');
  return sel ? parseInt(sel.value) : 500;
}

// ── Clear CSVs ────────────────────────────────────────────────────────────────
async function clearOutputCSVs() {
  if (!confirm('Delete all CSV and Consolidated files from the output folder? This cannot be undone.')) return;
  try {
    var r = await post('/api/clear_outputs', {});
    if (r.ok) {
      log('Cleared ' + r.deleted + ' CSV file(s) from output folder.', 'ok');
      toast('Cleared ' + r.deleted + ' CSV file(s)');
      if (r.errors && r.errors.length) log('Errors: ' + r.errors.join(', '), 'warn');
    } else {
      log('Clear CSVs failed: ' + r.error, 'err');
    }
  } catch(e) { log('Request failed: '+e.message, 'err'); }
}
...
// ── Basic Bundle ──────────────────────────────────────────────────────────────
var _bundlePoller = null;

function showBundle()  { $('bundle-overlay').classList.add('show'); }
function hideBundle()  {
  if (_bundlePoller) { clearInterval(_bundlePoller); _bundlePoller = null; }
  $('bundle-overlay').classList.remove('show');
}

function _resetBundleUI() {
  $('bundle-progress-bar').style.width = '0%';
  $('bundle-counter').textContent  = '0 / 0';
  $('bundle-current').textContent  = 'Initializing…';
  $('bundle-status').innerHTML     = '';
  $('bundle-skipped').textContent  = '';
  $('bundle-skipped').style.display = 'none';
  $('btn-bundle-close').disabled   = true;
}

async function startBasicBundle() {
  if (!_connected) return;
  _resetBundleUI();
  showBundle();
  try {
    var r = await post('/api/basic_bundle', {});
    if (!r.ok) {
      $('bundle-current').textContent = 'Error: ' + r.error;
      $('btn-bundle-close').disabled = false;
      return;
    }
    _bundlePoller = setInterval(pollBundleStatus, 1500);
  } catch(e) {
    $('bundle-current').textContent = 'Request failed: ' + e.message;
    $('btn-bundle-close').disabled = false;
  }
}
...
async function pollBundleStatus() {
  try {
    var resp = await fetch('/api/basic_bundle_status');
    var st   = await resp.json();
    var pct  = st.total > 0 ? Math.round(st.progress / st.total * 100) : 0;
    $('bundle-progress-bar').style.width = pct + '%';
    $('bundle-counter').textContent = st.progress + ' / ' + st.total;
    if (st.current) $('bundle-current').textContent = 'Running: ' + st.current;
    if (st.done) {
      clearInterval(_bundlePoller); _bundlePoller = null;
      $('bundle-progress-bar').style.width = '100%';
      $('bundle-counter').textContent = st.total + ' / ' + st.total;
      if (st.filename) {
        $('bundle-current').textContent = '✓ Completed — ' + st.ok_count + ' script(s) executed.';
        var skipNote = st.skip_count > 0 ? st.skip_count + ' skipped due to errors.<br>' : '';
        $('bundle-status').innerHTML = skipNote +
          '<a href="/output/' + encodeURIComponent(st.filename) + '" download="' + esc(st.filename) +
          '" style="color:var(--accent)">&#11015; Download ' + esc(st.filename) + '</a>';
        if (st.skipped && st.skipped.length) {
          $('bundle-skipped').textContent = st.skipped.join('\n');
          $('bundle-skipped').style.display = 'block';
        }
        log('Basic Bundle done: ' + st.ok_count + ' OK, ' + st.skip_count + ' skipped → ' + st.filename, 'ok');
        toast('Bundle ready: ' + st.filename);
      } else {
        $('bundle-current').textContent = 'Error writing output file.';
        if (st.error) $('bundle-status').textContent = st.error;
        log('Basic Bundle failed to write output.', 'err');
      }
      $('btn-bundle-close').disabled = false;
    }
...
if __name__ == "__main__":
    ...
    print(f"HANA SQL Console v6.0 running at {url}  (Ctrl+C to stop)")
