...
#log-wrap { height: 110px; flex-shrink: 0; border-top: 1px solid var(--border);
  overflow-y: auto; background: var(--surf); padding: 6px 12px;
...
/* ── About modal ── */
#about-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.75);
...
#btn-about { padding: 4px 10px; border-radius: 5px; border: 1px solid var(--border);
  background: var(--surf2); color: var(--muted); cursor: pointer; font-size: 12px; }

/* -- Consolidate modal -- */
#consolidate-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.75);
  display: none; align-items: center; justify-content: center;
  z-index: 300; backdrop-filter: blur(4px); }
...
#consolidate-box { background: var(--surf); border: 1px solid var(--border);
  ...
  box-shadow: 0 24px 56px rgba(0,0,0,.6); display: flex; flex-direction: column; gap: 14px; }
...
.output-item:hover { background: rgba(79,142,247,.08); }
  ...
  <div id="right-pane">
    ...
    <div class="tab-panel" id="panel-chart">
      <div id="chart-config">
        ...
        <div class="tb-sep"></div>
        <label>Color</label>
        <input type="color" id="cfg-color" value="#4f8ef7" onchange="buildChart()"/>
        <div class="tb-sep"></div>
        <label>Title</label>
